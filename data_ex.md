...
## تدقيق سريع للأرقام (Sanity Check)

> القيم التالية ناتجة من تشغيل `extract_posts("posts_linkedin.txt")` على الملف الحالي.

| المؤشر | القيمة |
|---|---:|
| عدد الصفوف `posts` | 48 |
| إجمالي `impressions` | 20,224 |
| إجمالي `likes` | 147 |
| إجمالي `comments` | 20 |
| إجمالي `reposts` | 2 |

مهم:
- عمود `reposts` هنا يمثل **عدد إعادة نشر الجمهور للمنشور** كما ظهر في سطر التحليلات داخل ملف النص.
- إذا كان الملف المصدر لا يحتوي إلا سطور قليلة من نوع `X repost` فالإجمالي سيكون منخفضًا حتى لو كانت هناك منشورات من نوع `reposted this`.
